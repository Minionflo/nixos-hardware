...
in
{
  ...
  options = {
    ...
    hardware.framework.amd-7040.preventWakeOnTouchpad = lib.mkOption {
      type = lib.types.bool;
      default = false;
      description = ''
        Stop the system from waking from suspend when the touchpad is clicked.

        See:
        https://community.frame.work/t/tracking-framework-amd-ryzen-7040-series-lid-wakeup-behavior-feedback/39128/104
      '';
    };
  };
  ...
  config = {
    ...
    services.udev.extraRules = lib.optionalString (lib.versionOlder config.boot.kernelPackages.kernel.version "6.7" && cfg.preventWakeOnAC) ''
      # Prevent wake when plugging in AC during suspend. Trade-off: keyboard wake disabled. See:
      # https://community.frame.work/t/tracking-framework-amd-ryzen-7040-series-lid-wakeup-behavior-feedback/39128/45
      ACTION=="add", SUBSYSTEM=="serio", DRIVERS=="atkbd", ATTR{power/wakeup}="disabled"
    '' + (lib.optionalString (cfg.preventWakeOnTouchpad) ''
